...
class UniqueClient extends Client {
    ...
    constructor({token, updateInterval=1000 * 60 * 30}: UniqueOptions, options?: ClientOptions) {
        super(options)
        this._token = token
    ...
    private async updateGuilds() {
        const res = await this._bot.updateGuilds(
            this.shard ? await this.shard.fetchClientValues('guilds.cache.size').then(res => res.reduce((acc, cur) => acc + cur)) : this.guilds.cache.size
        )
        this.emit('updateGuilds', res.botAccount.guilds)
    }
}
